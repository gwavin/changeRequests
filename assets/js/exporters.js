(function () {
  ...
  function valueOrBlank(value) {
    ...
    if (value && typeof value === "object" && value.dataUrl) return value.name || "Attached image";
  ...
  function htmlImageDefinition(label, value) {
    if (!value || !value.dataUrl) return "";
    return "<div class=\"def image-def\"><dt>" + escapeHtml(label) + "</dt><dd><figure class=\"attached-image\"><img src=\"" + escapeHtml(value.dataUrl) + "\" alt=\"" + escapeHtml(value.name || "Attached screenshot") + "\"><figcaption>" + escapeHtml(value.name || "Attached image") + "</figcaption></figure></dd></div>";
  }
  ...
  function ivSetBuiltPreview(item) {
    if (window.MnCmsIvPreview) {
      return window.MnCmsIvPreview.render(item);
    }
  ...
  function builtPreview(data, item) {
    if (data.typeId === "orderSentence" && window.MnCmsOsPreview) {
      return window.MnCmsOsPreview.render({ typeId: data.typeId, items: [item] });
    }
  ...
  function itemHtml(data, fields, item, index) {
    ...
    fields.forEach(function (field) {
      ...
      if (field.type === "image") {
        if (item[field.key] && item[field.key].dataUrl) current.rows.push(htmlImageDefinition(field.label, item[field.key]));
      } else {
        var value = valueOrBlank(item[field.key]).trim();
        if (value) {
  ...
  function html(data, fields) {
    ...
    var style = "body{margin:0;background:#eef2f4;color:#17212b;font-family:Segoe UI,Arial,sans-serif;line-height:1.5}main{max-width:1120px;margin:0 auto;padding:32px 20px 56px}.hero{background:#123d5a;color:white;padding:30px;border-radius:10px;margin-bottom:20px}.hero h1{margin:0 0 8px;font-size:30px;line-height:1.15}.hero p{margin:0;color:#d7e7ef}.meta{display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:10px;margin-top:18px}.meta div,.panel,.item{background:white;border:1px solid #d8e0e5;border-radius:8px;box-shadow:0 8px 22px rgba(23,33,43,.08)}.meta div{padding:12px;color:#17212b}.meta span,.ehr-title span,.item header p{display:block;color:#5c6b76;font-size:12px;font-weight:700;text-transform:uppercase}.meta strong{display:block;margin-top:3px}.panel{padding:18px;margin:16px 0}.panel h2,.detail-section h3{margin:0 0 10px;color:#123d5a}.item{margin:18px 0;overflow:hidden}.item header{padding:16px 18px;background:#f8fafb;border-bottom:1px solid #d8e0e5}.item header h2{margin:0;color:#123d5a}.ehr-preview{margin:16px;border:1px solid #9fb4c4;border-radius:8px;background:#fff;overflow:auto}.ehr-title{display:flex;justify-content:space-between;gap:12px;padding:10px 12px;background:#1f4e79;color:white}.ehr-title span{color:#dcecf8}.ehr-order-name{padding:14px 12px;font-size:18px;font-weight:800;color:#123d5a}.ehr-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:1px;background:#d4e0e8;border-top:1px solid #d4e0e8}.ehr-line{background:white;padding:10px}.ehr-line span{display:block;color:#60717d;font-size:12px}.ehr-line strong{display:block;color:#17212b}.ehr-subtitle{padding:9px 12px;background:#24527c;color:white;font-weight:800}.ehr-table,.iv-built,.iv-template{width:100%;border-collapse:collapse;background:white}.ehr-table th,.iv-built th.blue,.iv-template th{background:#2f6da8;color:white;text-align:left;padding:8px;font-size:13px}.iv-built th.left{width:26%}.iv-built td,.iv-template td,.ehr-table td{border:1px solid #9aa9b5;padding:8px;vertical-align:top;min-height:30px}.iv-built .built-name{font-weight:800;color:#123d5a}.iv-template{margin-top:14px}.iv-template th.section{background:#1f4e79;font-size:15px}.iv-template small,.iv-built small{font-weight:500;color:#e9f2f8}.ehr-note{margin:10px 12px 12px;padding:10px;background:white;border:1px solid #cbd8e1;border-radius:6px}.detail-section{padding:0 18px 16px}.detail-section dl{display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:10px;margin:0}.def{padding:10px;border:1px solid #d8e0e5;border-radius:6px;background:#fbfcfd}.def dt{font-weight:800;color:#304555}.def dd{margin:4px 0 0}.image-def{grid-column:1/-1}.attached-image{margin:0}.attached-image img{display:block;max-width:100%;max-height:620px;border:1px solid #cbd8e1;border-radius:6px}.attached-image figcaption{margin-top:6px;color:#5c6b76;font-size:12px}.empty{color:#8a98a3;font-style:italic}.governance{border-left:5px solid #176b5f}@media print{body{background:white}main{padding:0}.hero,.item,.panel{box-shadow:none}.ehr-preview{overflow:visible}}";
    style += ".iv-draft-preview{margin:16px;overflow:hidden;border:1px solid #9fb4c4;border-radius:8px;background:#fff}.iv-preview-title{display:flex;align-items:center;justify-content:space-between;gap:16px;padding:12px 14px;color:#fff;background:#1f4e79}.iv-preview-title span,.iv-preview-title strong{display:block}.iv-preview-title span{color:#dcecf8;font-size:11px;font-weight:800;text-transform:uppercase}.iv-preview-title small{color:#dcecf8}.iv-preview-scroll{overflow-x:auto;padding:12px}.iv-summary-table,.iv-detail-table{width:100%;min-width:760px;border-collapse:collapse;font-size:12px}.iv-detail-table{margin-top:12px}.iv-summary-table th,.iv-detail-table th{padding:8px;color:#fff;background:#2f6da8;border:1px solid #9aa9b5;text-align:left}.iv-summary-table td,.iv-detail-table td{padding:8px;border:1px solid #9aa9b5;vertical-align:top}.iv-summary-table .iv-description{color:#123d5a;font-weight:800}.iv-detail-table .iv-section-heading{background:#1f4e79}.iv-empty{color:#7b8790;font-style:italic;font-weight:400}@media(max-width:700px){.iv-preview-title{align-items:flex-start;flex-direction:column}}";
    style += ".iv-system-preview{color:#111;border-color:#858585;border-radius:0;font-family:Arial,sans-serif;font-size:12px}.iv-preview-warning{padding:7px 10px;color:#704d00;background:#fff4ce;border-bottom:1px solid #d6b656;font-weight:700}.iv-system-title{padding:5px 7px;border-bottom:1px solid #b7b7b7;font-size:18px;font-weight:700}.iv-system-tabs{display:flex;background:#f5f5f5;border-bottom:1px solid #aaa}.iv-system-tabs button{padding:5px 12px;border:0;border-right:1px solid #aaa;background:#f5f5f5;font-weight:700}.iv-system-panel{min-height:180px;padding:14px 10px;border-bottom:1px solid #aaa}.iv-system-form{display:grid;grid-template-columns:1fr 1fr;gap:40px}.iv-system-form>div{display:grid;gap:8px;align-content:start}.iv-system-form label{display:grid;grid-template-columns:130px 1fr;align-items:center;gap:7px;text-align:right}.iv-continuous-table{width:100%;min-width:720px;border-collapse:collapse}.iv-continuous-table th{padding:3px 5px;background:#d9e7f4;border:1px solid #777;font-weight:400;text-align:left}.iv-continuous-table td{height:28px;padding:3px 5px;background:#fffed6;border:1px solid #888}.iv-continuous-table tr:nth-child(1) td{background:#dcc4ad}.iv-row-marker{background:#d9e7f4!important}.iv-result-strip{display:grid;grid-template-columns:.7fr .7fr 1.25fr 1.6fr;gap:14px;margin-top:12px}.iv-result-strip label,.iv-instruction-label{display:grid;gap:5px;margin-top:18px}.iv-base-help{margin:10px;border:1px solid #b8c8d5;background:#f4f8fb}.iv-base-help summary,.iv-base-help p{padding:8px 10px}.iv-derived-note{color:#59636a}@media(max-width:700px){.iv-system-form{grid-template-columns:1fr;gap:8px}.iv-result-strip{grid-template-columns:1fr 1fr}}";
    style += ".os-live-preview{margin:16px;padding-bottom:10px;color:#17212b;background:#f4f7f9;border:1px solid #aab8c3;border-radius:8px;overflow:hidden;font-family:Arial,sans-serif;font-size:12px}.os-preview-kicker{margin:0;padding:8px 10px;color:#704d00;background:#fff4ce;border-bottom:1px solid #d6b656;font-weight:800}.os-preview-pair{display:grid;grid-template-columns:1fr 1fr;gap:10px;padding:10px}.os-preview-card{background:#fff;border:1px solid #858585}.os-preview-card h3{margin:0;padding:6px 8px;color:#111;background:#e5e7e9;border-bottom:1px solid #999}.os-system-title,.os-preview-subheading{padding:6px 8px;font-weight:800}.os-system-tabs{display:flex;border-block:1px solid #aaa;background:#f3f3f3}.os-system-tabs span{padding:5px 8px}.os-oef-grid{display:grid;grid-template-columns:1fr 1fr;gap:7px;padding:9px}.os-oef-field{display:grid;grid-template-columns:1fr 1fr;gap:5px}.os-preview-value,.os-display-line{display:block;padding:5px;color:#111;background:#fffec9;border:1px solid #999}.os-preview-empty{color:#68727a;background:#f3f3f3;font-style:italic}.os-order-comments,.os-display-line,.os-cdl-row{margin:8px}.os-cdl-row{display:grid;grid-template-columns:auto 1fr;gap:12px;padding:7px;border:1px solid #999}@media(max-width:700px){.os-preview-pair{grid-template-columns:1fr}}";
