...
Discussion-ready requests can also open a prepared email to the MN-CMS Meds Agenda. Its body contains the readable request followed by a delimited, versioned JSON representation for future automation. Images are represented by metadata only and are not attached. The user must review the draft and press Send in their mail client; the builder does not send email automatically.
